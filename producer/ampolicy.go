...
func DeletePoliciesPolAssoIdProcedure(polAssoId string) *models.ProblemDetails {
	ue := pcfContext.PCF_Self().PCFUeFindByPolicyId(polAssoId)
...
func UpdatePostPoliciesPolAssoIdProcedure(polAssoId string,
	...
	if policyAssociationUpdateRequest.GetNotificationUri() != "" {
		amPolicyData.NotificationUri = policyAssociationUpdateRequest.GetNotificationUri()
	}
...
func PostPoliciesProcedure(polAssoId string,
	...
	if amPolicy == nil || amPolicy.AmPolicyData == nil {
		...
		apiReadAccessAndMobilityPolicyDataRequest := client.AccessAndMobilityPolicyDataDocumentAPI.ReadAccessAndMobilityPolicyData(context.Background(), ue.Supi)
	...
	reqSuppFeat := pcfSelf.PcfSuppFeats[models.SERVICENAME_NPCF_AM_POLICY_CONTROL]
